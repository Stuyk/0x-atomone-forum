...
export async function getCurrentBlockHeight() {
  try {
    const response = await fetch(
      `${config.ATOM_ONE_API_URL}/cosmos/base/tendermint/v1beta1/blocks/latest`
    );
    if (!response.ok) {
...
export async function getBlockByHeight(blockHeight: number) {
  try {
    const response = await fetch(
      `${config.ATOM_ONE_API_URL}/cosmos/base/tendermint/v1beta1/blocks/${blockHeight}`
    );
    if (!response.ok) {
      console.log(response);
      throw new Error(`Failed to fetch block: ${response.statusText}`);
...
export async function getMemoFromTx(txHash: string, timestamp: string) {
  try {
    const txResponse = await fetch(
      `${config.ATOM_ONE_API_URL}/cosmos/tx/v1beta1/txs/${txHash.toUpperCase()}`
    );
